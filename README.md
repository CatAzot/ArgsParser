...
* Разбор опций с нефиксированным числом параметров. Парсинг будет производится до следующего ключа или опции, либо до конца всех аргументов (пример: `user@userpc:~$ programName --unfix param1 param2 param3 -u paramA paramB`).
...
private val clParser = KArgsParser(
    ...
    // some action with params
    true 
}
```

​	Все параметры, за исключением последнего, используются только при построении подскзки по использованию программы следующего вида:
...
val clParser = KArgsParser { 
    params ->
    // some action with params  // 9
    true
}
~~~

​	В качестве параметра в (8) и (9) должно приниматься лямбда-выражения со следующей сигнатурой:

~~~kotlin
val applyParams: (Array<String>) -> Boolean
~~~

​	Данная функция будет использована при разборе параметров программы, т.е. всех значений, которые не являются ключами, опциями или их параметрами. Пример использования:
...
​	Небольшая программа демонстрирующая алгоритм использования парсера:
